...
if [[ -n "$LERNHIVE_DOMAIN" ]]; then
  ...
  # Apache inside the webserver container listens on port 80. Port 8000 is
  # only the host-side mapping (8000→80); Caddy reaches the container via
  # the internal Docker network so it must use port 80.
  MOODLE_UPSTREAM="${MOODLE_COMPOSE_SERVICE}:80"
