...
- **Admin override UI.** *(new in 0.3.0, ADR-01)* Settings page under `Site administration → LernHive → Level configuration`. Admins can set each feature to `Default`, `Disabled`, or an explicit level (1–5), backed by `override_store`.
- **Flavor presets.** *(new in 0.3.0, ADR-01)* `local_lernhive_flavour` now calls `registry::apply_flavor_preset(...)` during flavor activation. Manual admin overrides continue to win over flavor presets.
...
## Feature-to-Level default map *(ADR-01)*

The canonical map lives in `local_lernhive/classes/feature/registry.php` (`registry::build_features()`). High-level summary:
