# theme_lernhive — User Documentation

## User value
The theme makes LernHive feel calmer, clearer, and easier to navigate without changing Moodle core behavior.

## What the user should experience
- simple, clear labels
- minimal steps
- no hidden complexity
- terminology that also works in product communication

## Release 1 experience
- navigation feels left-oriented and predictable
- top navigation shows only a few important actions
- top navigation stays anchored to the viewport while scrolling — profile, settings, logout, notifications, launcher, and language picker remain reachable without scrolling back to the top
- on a course page, the left sidebar shows a focused "Course navigation" section (with a small sitemap icon) under the primary nav — listing the course sections and activities with the current activity gently highlighted; no skeleton flash on page load, no uppercase labels, no stray rectangular buttons
- Launcher feels like a quick action surface, not a second menu system
- Launcher starts from a single icon-like trigger and opens a compact action flyout
- cards, lists, and forms feel calmer and easier to scan than standard Moodle
- the same visual language works across School and the optional LXP Flavour
- Follow can appear on cards as a star action, while Bookmark remains separate and explicit
- course pages keep the learning content central and place helper surfaces in a quieter secondary area
- short-form Snack surfaces can feel lighter, more linear, and more expectation-safe than standard course pages

## Main user journeys
- open a page and understand the main structure immediately
- move between areas through a stable left navigation
- use the Launcher for creation and admin actions without losing orientation
- read cards, tiles, and helper surfaces without visual overload
- use the same interface comfortably on desktop and smaller screens
- understand common action patterns consistently across Explore, ContentHub, and reporting surfaces
- stay oriented on course pages without a visually dominant secondary navigation pattern

## Mockup goal
- the mockup should already show how Moodle pages, LernHive cards, and action surfaces fit into one coherent theme system
