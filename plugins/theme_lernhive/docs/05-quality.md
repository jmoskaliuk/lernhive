...
- verify the sticky top page header (since 0.9.53):
  ...
  - on a Plugin Shell page (e.g. ContentHub, Copy) Zone 0 remains visible above Zone A/Zone B at page load; on scroll, both the theme page header and `.lh-plugin-header` are sticky and must stack cleanly — no overlap, no gap with content peeking through
